...
exports.get_recommended_workouts = (req, res) => {
  ...
  db.manyOrNone("SELECT DISTINCT user_id FROM user_liked_workout")
    ...
    .then((matrix) => {
      db.manyOrNone("SELECT user_id, workout_id FROM user_liked_workout").then(
        (data) => {
          ...
          if (matrix[userId] === undefined) {
            console.log(
              "Error: User: " + userId + " has not liked any workouts "
            );
            res.send([]);
          } else {
            ...
            let similarUsers = similarities.filter(
            ...
            for (var workout_id in likedWorkoutUsersSimilarityScore) {
              var sumOfSimilarity = likedWorkoutUsersSimilarityScore[
                workout_id
              ].reduce((sum, val) => {
                return (sum += val);
              }, 0);
              ...
              probabilityOfLikingWorkout[workout_id] =
                sumOfSimilarity /
                likedWorkoutUsersSimilarityScore[workout_id].length;
            }
            ...
            if (reccomendedWorkoutIds.length == 0) {
              ...
              let query =
                "SELECT * FROM workout WHERE workout_id IN (" +
